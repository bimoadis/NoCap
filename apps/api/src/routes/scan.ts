...
export async function scanRoutes(fastify: FastifyInstance) {
  fastify.route({
    ...
    handler: async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      const query = (req.query || {}) as { mint?: string; chain?: string; stream?: string };
      
      const mint = body.mint || query.mint;
      const stream = body.stream !== undefined ? body.stream : (query.stream === 'true');
